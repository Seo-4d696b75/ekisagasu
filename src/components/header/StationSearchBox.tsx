...
const StationSearchBox: FC<SearchProps> = ({ onSuggestionSelected, inputFocusRequested }) => {
  ...
  const onSuggestionsFetchRequested = useRefCallback((request: Autosuggest.SuggestionsFetchRequestedParams) => {
    ...
    lastRequestIdRef.current = setTimeout(() => {
      ...
      setLoading(true)
      Promise.all([
        axios.get<ApiResponse[]>(`${process.env.VITE_STATION_API_URL}/station/search?name=${value}&extra=${isDataExtra}`),
        axios.get<ApiResponse[]>(`${process.env.VITE_STATION_API_URL}/line/search?name=${value}&extra=${isDataExtra}`)
      ]).then(res => {
