...
describe("StationRepository", () => {
  ...
  const path = `${import.meta.env.VITE_STATION_API_URL}/info`
  ...
  describe("駅・路線データ読み出し", () => {
    ...
    test("getStationOrNull > API > not found", async () => {
      const path = `${import.meta.env.VITE_STATION_API_URL}/station?code=${target.code}`
    ...
    test("getStationById > API > not found", async () => {
      const path = `${import.meta.env.VITE_STATION_API_URL}/station?id=${target.id}`
      mock.onGet(path).reply(404)
